...
- **A menu bar appears whenever the Main Window is active**, because a Regular app owns one. It is deliberately two menus — the app menu with About, Settings, Close Window and Quit, and Edit. Only the app menu belongs to the Main Window: those are the commands a Regular application owns and an Accessory one does not, so they arrive with the window and leave with it.
- **Edit belongs to the application, not to the Main Window**, and is installed once at startup and never removed. macOS routes Cmd+C/V/X/Z through menu items, so an app with no menu has no clipboard and no undo anywhere — including in the resident Capture and Task Creation panels, which exist with no Main Window at all.
