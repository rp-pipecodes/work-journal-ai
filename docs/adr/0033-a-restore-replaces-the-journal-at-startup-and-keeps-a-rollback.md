...
- **Every future migration raises the version boundary a restore refuses on, and must keep older snapshots migratable.** The boundary is the newest migration this build ships — `the_supported_version_tracks_the_migrations_list` holds the constant and the list together, and `expected_tables_cover_the_real_schema` holds the table check against the production schema; review attention belongs on the order of the renames and the sidecar handling, which is the whole safety argument.
