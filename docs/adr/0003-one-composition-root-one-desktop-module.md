...
- **The constants are still duplicated in Rust, and the shared strings are checked.** They are one screen each rather than one screen total, and each carries a "must match" comment. `src/platform/desktop-rust.test.ts` reads `src-tauri/src/lib.rs` as text and fails when a shared string, or a "must match" comment naming one, has no counterpart. The geometry constants stay unchecked on purpose: they are only the size a resident window is built at before its webview boots, and the view fits the window from the TypeScript numbers on mount.
