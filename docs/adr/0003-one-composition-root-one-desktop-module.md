...
- **The constants are still duplicated in Rust, and every string declared here is checked against its copy.** They are one screen each rather than one screen total, and each carries a "must match" comment. `src/platform/desktop-rust.test.ts` reads `src-tauri/src/lib.rs` as text and fails when a string declared in `desktop.ts`, or a "must match" comment naming one, has no matching counterpart. Two things stay unchecked on purpose: the geometry constants, which are only the size a resident window is built at before its webview boots and which the view then fits from the TypeScript numbers; and the settings keys that are declared in `src/settings/settings.ts` rather than here, which is the seam this ADR describes not being held to yet.
