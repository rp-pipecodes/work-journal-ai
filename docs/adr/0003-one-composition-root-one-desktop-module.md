...
- **The shared strings are still duplicated in Rust.** `src/platform/desktop-rust.test.ts` reads both source files and checks that every string named by a "must match" comment agrees; the geometry constants remain an intentional exception.
