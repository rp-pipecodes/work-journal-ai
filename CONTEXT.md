...
**Standup Prompt**:
The system prompt a Standup Post is written under. Not Model Access but voice: what the post should sound like and what shape it takes, so the user can make it match the chat group they actually post to. Plain text with a shipped default that can be restored, and empty means that default rather than no instruction — a model asked nothing does not write a standup post.
_Avoid_: Prompt template, instructions, AI config
